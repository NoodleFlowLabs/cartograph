...
export function getInvariants(data) {
  ...
  return { results: arr(invariants, 'results'), summary: record(invariants, 'summary') }
}
...
export function linkedItems(data, key, ids) {
  return ids.map((id) => findById(arr(data, key), id)).filter(Boolean)
}
...
export function implementationLabel(item) {
  ...
  return [text(implementation, 'file'), text(implementation, 'function')].filter(Boolean).join(' -> ')
}
