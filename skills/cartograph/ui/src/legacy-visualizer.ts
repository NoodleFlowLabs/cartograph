  ...
  `;
      ...
      function renderFileMapPrompt(feature) {
        ...
        const entityNames = (feature.entityIds || [])
          ...
          .join(", ");
        const jsonPath = "cartograph.json";
...
Map every file in the codebase that participates in the "${feature.name}" feature and update the cartograph JSON.
...
> This context was generated by the **Cartograph** skill (codebase mapping tool, defined in \`.agents/skills/cartograph/\`), from \`cartograph.json\`. See \`cartograph.json\` for the full codebase map — surfaces, features, entities, relationships, operations, flows, and compartments.
  ...
  `;
    ...
    <div class="codemap-header" style="position:relative;">
      <div style="display:flex;gap:8px;position:absolute;top:0;right:0;">
        <button class="codemap-toggle-btn${codemapView === "tree" ? " active" : ""}" onclick="showCodemapTree('${comp.id}')">Tree View</button>
    ...
    <div class="surface-section">
      ...
      <div class="surface-entity-list">
        ${children
          .map(
            (c) => `
          <div class="surface-entity-item" onclick="expandAndSelectCompartment('${comp.id}', '${c.id}')">
      ...
      function loadData(json) {
        try {
          ...
          if (
            ...
            throw new Error(
              "Missing required fields: meta, entities, relationships, operations, flows",
            );
          }
          ...
          if (!DATA.fileTree) DATA.fileTree = [];
...
let localFileFallbackInitialized = false;
function setupLocalFileFallback() {
  if (localFileFallbackInitialized) return;
  const dropZone = document.getElementById("drop-zone");
  const fileInput = document.getElementById("file-input") as HTMLInputElement | null;
  const browseBtn = document.getElementById("browse-btn");
  if (!dropZone || !fileInput || !browseBtn) return;
  localFileFallbackInitialized = true;
...
function showCodemapTree(compartmentId: string) {
  codemapView = "tree";
  renderCompartmentDetail(compartmentId);
}

function expandAndSelectCompartment(parentId: string, childId: string) {
  expandedCompartments[parentId] = true;
  selectCompartment(childId);
}

export function mountVisualizer() {
  exposeLegacyHandlers();
  setupLocalFileFallback();
  loadDataFromServer();
  remoteCleanup = subscribeToCartographChanges(() => {
    loadDataFromServer();
  });
  return () => {
