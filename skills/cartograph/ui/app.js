import { html, render, useCallback, useEffect, useMemo, useState } from './vendor/htm-preact-standalone.module.js'
import { InvariantBadge, Stat } from './components/common.js'
import { Panel, Sidebar } from './components/layout.js'
...
function App() {
  ...
  const displayTab = visibleTabs.some((tab) => tab.id === activeTab) ? activeTab : 'overview'
  ...
  const loadCartograph = useCallback(async () => {
    try {
      ...
      if (response.status === 404) {
        setLoadState({
          status: 'missing',
          message: text(body, 'error') || 'No cartograph.json found. Run /cartograph from your agent.',
        })
        return
      }
    ...
    <main className="app-shell">
      ...
      <nav className="tabs" aria-label="Cartograph sections">
        ${['overview', 'pm', 'eng'].map((group) => {
          ...
          return html`
            <div
              className=${`tab-cluster ${groupTabs.some((tab) => tab.id === displayTab) ? 'active-group' : ''}`}
              data-group=${group}
              key=${group}
            >
              ${group !== 'overview' ? html`<span className="tab-group-label">${group}</span>` : null}
              <div className="tab-cluster-items">
                ${groupTabs.map((tab) => html`
                  <button className=${`tab ${tab.id === displayTab ? 'active' : ''}`} key=${tab.id} onClick=${() => switchTab(tab.id)} type="button">
                    ${tab.label}
