    ...
    <main class="app-shell">
      ...
      <nav class="tabs" aria-label="Cartograph sections">
        ${['overview', 'pm', 'eng'].map((group) => {
          ...
          return html`
            <div class=${`tab-cluster ${groupTabs.some((tab) => tab.id === displayTab) ? 'active-group' : ''}`} data-group=${group} key=${group}>
              ${group !== 'overview' ? html`<span class="tab-group-label">${group}</span>` : null}
              <div class="tab-cluster-items">
                ${groupTabs.map((tab) => html`
                  <button class=${`tab ${tab.id === displayTab ? 'active' : ''}`} key=${tab.id} onClick=${() => switchTab(tab.id)} type="button">
                    ${tab.label}
