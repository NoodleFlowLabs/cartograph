...
export function Welcome({
  ...
  return html`
    <main class="welcome-shell">
      ...
      <div class=${`drop-zone ${dragging ? 'dragover' : ''}`} onDragLeave=${onDragLeave} onDragOver=${onDragOver} onDrop=${onDrop}>
        <div class="drop-zone-title">Drop <code>cartograph.json</code> here</div>
        <div class="drop-zone-sub">Or use the local server with <code>node skills/cartograph/server.mjs</code></div>
        <label class="drop-zone-btn">
          Browse files
          <input accept=".json,application/json" hidden onChange=${(event) => onFile(event.target.files?.[0])} type="file" />
        </label>
        <button class="secondary-btn" onClick=${onRefresh} type="button">Refresh local data</button>
