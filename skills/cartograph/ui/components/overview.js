...
export function Overview({ data, switchTab }) {
  ...
  return html`
    <div className="overview-stack">
          ...
          <div>
            ...
            <div className="health-row">
              ${metrics.map((metric) => html`
                <button className="health-pill" key=${itemId(metric)} onClick=${() => switchTab('codehealth')} type="button">
                  <span className=${`score-dot ${metricState(metric)}`}></span>
        ...
        <section className="overview-strip">
          <div>
            ...
            <button className="health-pill" onClick=${() => switchTab('invariants')} type="button">
              <span className=${`score-dot ${num(invariants.summary, 'failing') > 0 ? 'red' : 'green'}`}></span>
      ...
      ${techStack.length ? html`
        <section className="overview-strip">
          <div>
            <div className="overview-section-title">Tech stack</div>
            <div className="chip-row">
              ${techStack.slice(0, 10).map((item) => html`
                <button className="detail-chip" key=${itemId(item)} onClick=${() => switchTab('techstack')} type="button">
      ...
      <section>
        ...
        <div className="overview-card-grid">
          ${surfaces.map((surface, index) => html`
            <button className="overview-card" key=${itemId(surface, index)} onClick=${() => switchTab('surfaces', itemId(surface, index))} type="button">
      ...
      <section>
        ...
        <div className="overview-card-grid two">
          ${flows.slice(0, 8).map((flow, index) => html`
            <button className="overview-card" key=${itemId(flow, index)} onClick=${() => switchTab('flows', itemId(flow, index))} type="button">
